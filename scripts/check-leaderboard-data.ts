import { db } from '../server/db.js';
import { predictions, users, userProfiles } from '../shared/schema.js';
import { eq, and, gte, lte, inArray } from 'drizzle-orm';

async function checkLeaderboardData() {
  try {
    const now = new Date();
    const year = now.getFullYear();
    const month = now.getMonth();
    const startOfMonth = new Date(Date.UTC(year, month, 1, 0, 0, 0, 0));
    const endOfMonth = new Date(Date.UTC(year, month + 1, 0, 23, 59, 59, 999));
    
    console.log('=== CURRENT MONTH LEADERBOARD CHECK ===\n');
    console.log('Current month:', now.toISOString().slice(0, 7));
    console.log('Start:', startOfMonth.toISOString());
    console.log('End:', endOfMonth.toISOString());
    console.log('');
    
    // Get predictions for current month
    const preds = await db
      .select({
        userId: predictions.userId,
        status: predictions.status,
        timestampCreated: predictions.timestampCreated,
        pointsAwarded: predictions.pointsAwarded,
        result: predictions.result,
        evaluatedAt: predictions.evaluatedAt,
      })
      .from(predictions)
      .where(
        and(
          inArray(predictions.status, ['active', 'evaluated']),
          gte(predictions.timestampCreated, startOfMonth),
          lte(predictions.timestampCreated, endOfMonth)
        )
      );
    
    console.log(`📊 Total predictions found: ${preds.length}\n`);
    
    // Aggregate by user
    const userStats = new Map<string, {
      totalScore: number;
      totalPredictions: number;
      correctPredictions: number;
      evaluatedPredictions: number;
    }>();
    
    for (const pred of preds) {
      if (!userStats.has(pred.userId)) {
        userStats.set(pred.userId, {
          totalScore: 0,
          totalPredictions: 0,
          correctPredictions: 0,
          evaluatedPredictions: 0,
        });
      }
      
      const stats = userStats.get(pred.userId)!;
      stats.totalPredictions++;
      
      if (pred.status === 'evaluated') {
        stats.evaluatedPredictions++;
        stats.totalScore += pred.pointsAwarded || 0;
        if (pred.result === 'correct') {
          stats.correctPredictions++;
        }
      }
    }
    
    console.log(`👥 Unique users with predictions: ${userStats.size}\n`);
    
    // Get user details
    const userIds = Array.from(userStats.keys());
    const usersData = await db
      .select({
        id: users.id,
        username: users.username,
        role: users.role,
      })
      .from(users)
      .where(inArray(users.id, userIds));
    
    const userMap = new Map(usersData.map(u => [u.id, u]));
    
    // Display user stats
    console.log('=== USER STATISTICS ===\n');
    for (const [userId, stats] of userStats.entries()) {
      const user = userMap.get(userId);
      console.log(`User: ${user?.username || 'Unknown'} (${user?.role || 'user'})`);
      console.log(`  - Total Predictions: ${stats.totalPredictions}`);
      console.log(`  - Evaluated: ${stats.evaluatedPredictions}`);
      console.log(`  - Correct: ${stats.correctPredictions}`);
      console.log(`  - Total Score: ${stats.totalScore}`);
      console.log('');
    }
    
    // Check user profiles
    console.log('=== USER PROFILES CHECK ===\n');
    const profiles = await db
      .select({
        userId: userProfiles.userId,
        monthlyScore: userProfiles.monthlyScore,
        totalPredictions: userProfiles.totalPredictions,
        correctPredictions: userProfiles.correctPredictions,
      })
      .from(userProfiles)
      .where(inArray(userProfiles.userId, userIds));
    
    for (const profile of profiles) {
      const user = userMap.get(profile.userId);
      console.log(`Profile: ${user?.username || 'Unknown'}`);
      console.log(`  - Monthly Score: ${profile.monthlyScore}`);
      console.log(`  - Total Predictions: ${profile.totalPredictions}`);
      console.log(`  - Correct Predictions: ${profile.correctPredictions}`);
      console.log('');
    }
    
  } catch (error) {
    console.error('❌ Error:', error);
  } finally {
    process.exit(0);
  }
}

checkLeaderboardData();

