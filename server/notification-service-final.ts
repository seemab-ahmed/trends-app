...
function setupBrevoApi() {
  ...
  try {
    ...
    // Utilizziamo l'enum corretto per impostare la chiave API
    ...
    if (apiInstance) {
      ...
      try {
        ...
        if (sendError.statusCode === 401 && sendError.body && sendError.body.message) {
          console.error(`Autorizzazione API Brevo fallita: ${sendError.body.message}`);
          console.error("Per risolvere questo problema, accedi al tuo account Brevo e autorizza l'indirizzo IP al link: https://app.brevo.com/security/authorised_ips");
        } else {
          console.error("Errore nell'invio dell'email:", sendError);
        }
        return false;
      }
    }
